...
OUTPUT_RAW_CSV = "mini_pot_raw_crawled.csv"
OUTPUT_COMPARE_CSV = "mini_pot_sentiment_compare.csv"
OUTPUT_BRAND_CSV = "mini_pot_brand_candidates.csv"
OUTPUT_BRAND_MAPPING_CSV = "mini_pot_brand_mapping.csv"
OUTPUT_COLLECTED_BRANDS_CSV = "mini_pot_collected_brands.csv"
OUTPUT_OVERVIEW_CSV = "mini_pot_assignment_overview.csv"
OUTPUT_SUMMARY_CSV = "mini_pot_assignment_summary.csv"
OUTPUT_SENTIMENT_SUMMARY_CSV = "mini_pot_sentiment_summary.csv"
...
BRAND_KEYWORD_MAP = {
    "다이소": ["다이소", "DAISO", "daiso"],
    "이케아": ["이케아", "IKEA", "ikea"],
    "모던하우스": ["모던하우스", "Modern House", "modern house"],
    "마켓비": ["마켓비", "MARKET B", "market b", "marketb"],
    "오늘의집": ["오늘의집", "Ohouse", "ohouse"],
    "한샘": ["한샘", "Hanssem", "hanssem"],
    "무인양품": ["무인양품", "MUJI", "muji"],
    "아트박스": ["아트박스", "ARTBOX", "artbox"],
    "텐바이텐": ["텐바이텐", "10x10", "텐바이텐 10x10"],
    "플라랜드": ["플라랜드", "Plaland", "plaland"],
    "갑조네": ["갑조네", "갑조네 화분"],
    "심폴": ["심폴", "Simpol", "simpol"],
    "마이플랜트": ["마이플랜트", "My Plant", "my plant", "myplant"],
    "그린테라피": ["그린테라피", "Green Therapy", "green therapy"],
    "화분월드": ["화분월드", "화분 월드"],
    "플라팜": ["플라팜", "Plafarm", "plafarm"],
    "보르미올리": ["보르미올리", "Bormioli", "bormioli"],
    "르chuza": ["레츄자", "르츄자", "LECHUZA", "lechuza"],
    "베란다레시피": ["베란다레시피", "베란다 레시피"],
    "식물나라": ["식물나라", "식물 나라"],
}
...
BRAND_STOPWORDS = {
    ...
    "화분", "미니화분", "미니 화분", "작은화분", "작은 화분", "소형화분", "소형 화분",
    "식물", "반려식물", "플랜트", "plant", "pot", "plants", "pots", "인테리어", "소품",
    "플라스틱", "세라믹", "도자기", "토분", "분갈이", "받침", "화분받침", "화분 받침",
}

# 브랜드가 직접 언급되지 않아도 상품/키워드 기반으로 후보군을 묶기 위한 맵핑입니다.
# 이 맵핑은 "확정 브랜드"가 아니라 "분석용 후보군"입니다.
PRODUCT_CATEGORY_KEYWORDS = {
    "미니 화분 계열": ["미니 화분", "미니화분", "mini pot", "mini plant pot", "small pot"],
    "소형 화분 계열": ["작은 화분", "작은화분", "소형 화분", "소형화분", "small planter"],
    "반려식물 계열": ["반려식물", "반려 식물", "식물 키우기", "plant care", "houseplant"],
    "다육이/선인장 계열": ["다육이", "다육", "선인장", "cactus", "succulent"],
    "인테리어 소품 계열": ["인테리어", "소품", "데스크테리어", "홈데코", "home decor", "desk decor"],
    "세라믹/도자기 화분 계열": ["세라믹", "도자기", "ceramic", "porcelain"],
    "토분 계열": ["토분", "테라코타", "terracotta"],
    "플라스틱 화분 계열": ["플라스틱", "plastic"],
    "분갈이/가드닝 계열": ["분갈이", "흙", "배양토", "가드닝", "gardening", "repotting"],
}
...
def brand_candidate_pipeline(raw_df: pd.DataFrame, brand_keyword_map: Optional[Dict[str, List[str]]] = None):
    ...
    3. 결과를 두 개 파일로 저장합니다.
       - mini_pot_brand_candidates.csv: 브랜드별 등장 빈도 요약
       - mini_pot_brand_mapping.csv: 게시글별 브랜드/상품군 맵핑 결과
    """
...
def save_assignment_csv_results(raw_df: pd.DataFrame, brand_df: pd.DataFrame, mapping_df: pd.DataFrame, compare_df: pd.DataFrame):
    ...
    생성 파일:
    1. mini_pot_assignment_overview.csv: 과제 결과 개요
    2. mini_pot_assignment_summary.csv: 과제 핵심 요약
    3. mini_pot_collected_brands.csv: 쇼핑 API 기반 브랜드 후보군 원본
    4. mini_pot_brand_candidates.csv: 브랜드 후보군 요약
    5. mini_pot_brand_mapping.csv: 게시글별 브랜드/상품군 맵핑 결과
    6. mini_pot_sentiment_summary.csv: 모델별 감정 라벨 분포
    7. mini_pot_sentiment_compare.csv: 게시글별 모델 감정 분석 결과
    8. mini_pot_raw_crawled.csv: 원본 크롤링 결과
    """
