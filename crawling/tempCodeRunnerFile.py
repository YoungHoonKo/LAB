...
OUTPUT_ASSIGNMENT_EXCEL = "aroma_oil_assignment_result.xlsx"
...
# =========================================================
def brand_candidate_pipeline(raw_df: pd.DataFrame):
    """
    ...
    3. 결과를 두 개 파일로 저장합니다.
       - aroma_oil_brand_candidates.csv: 브랜드별 등장 빈도 요약
       - aroma_oil_brand_mapping.csv: 게시글별 브랜드/상품군 맵핑 결과
    """
    if raw_df.empty:
...
def build_sentiment_summary(compare_df: pd.DataFrame) -> pd.DataFrame:
    """
    모델별 감정 라벨 분포를 엑셀 요약 시트용 DataFrame으로 만듭니다.
    """
    if compare_df.empty:
...
def build_assignment_overview(raw_df: pd.DataFrame, brand_df: pd.DataFrame, mapping_df: pd.DataFrame, compare_df: pd.DataFrame) -> pd.DataFrame:
    """
    과제 요구사항을 한눈에 보여주는 개요 시트용 DataFrame입니다.
    """
    crawled_ok_count = 0
    ...
    rows = [
        ...
        {"항목": "직접 브랜드 후보 수", "값": direct_brand_count, "설명": "브랜드 사전에서 직접 매칭된 후보 수"},
...
def autosize_excel_columns(writer: pd.ExcelWriter, sheet_name: str, df: pd.DataFrame, max_width: int = 45):
    """
    pandas ExcelWriter로 생성한 시트의 열 너비를 내용 길이에 맞게 조정합니다.
    openpyxl 엔진이 설치되어 있을 때 동작합니다.
    """
    worksheet = writer.sheets[sheet_name]
    for col_idx, column_name in enumerate(df.columns, start=1):
        values = [str(column_name)]
        if not df.empty:
            values.extend(df[column_name].astype(str).head(200).tolist())
...
def save_assignment_excel(raw_df: pd.DataFrame, brand_df: pd.DataFrame, mapping_df: pd.DataFrame, compare_df: pd.DataFrame):
    """
    과제 제출용 엑셀 파일을 생성합니다.

    생성 시트:
    1. overview: 과제 결과 개요
    2. brand_candidates: 브랜드 후보군 요약
    3. brand_mapping: 게시글별 브랜드/상품군 맵핑 결과
    4. sentiment_summary: 모델별 감정 라벨 분포
    5. sentiment_compare: 게시글별 모델 감정 분석 결과
    6. raw_crawled: 원본 크롤링 결과
    """
    ...
    with pd.ExcelWriter(OUTPUT_ASSIGNMENT_EXCEL, engine="openpyxl") as writer:
        overview_df.to_excel(writer, sheet_name="overview", index=False)
        brand_df.to_excel(writer, sheet_name="brand_candidates", index=False)
        mapping_df.to_excel(writer, sheet_name="brand_mapping", index=False)
        sentiment_summary_df.to_excel(writer, sheet_name="sentiment_summary", index=False)
        compare_df.to_excel(writer, sheet_name="sentiment_compare", index=False)
        raw_df.to_excel(writer, sheet_name="raw_crawled", index=False)

        sheet_data = {
            "overview": overview_df,
            "brand_candidates": brand_df,
            "brand_mapping": mapping_df,
            "sentiment_summary": sentiment_summary_df,
            "sentiment_compare": compare_df,
            "raw_crawled": raw_df,
        }
    ...
    print(f"[6] 과제 제출용 엑셀 저장 완료: {OUTPUT_ASSIGNMENT_EXCEL}")
...
if __name__ == "__main__":
    ...
    brand_df, mapping_df = brand_candidate_pipeline(raw_df)
    ...
    save_assignment_excel(raw_df, brand_df, mapping_df, compare_df)
    ...
    print(f"- 과제 제출용 엑셀: {OUTPUT_ASSIGNMENT_EXCEL}")
